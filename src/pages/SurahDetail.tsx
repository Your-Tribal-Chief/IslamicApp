...
export default function SurahDetail() {
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#f5f5f0] pb-8">
      {/* Header */}
      <div className="sticky top-0 z-50 bg-emerald-800 text-white p-4 flex items-center shadow-md">
      ...
      {surah?.number !== 1 && surah?.number !== 9 && (
        <div className="py-8 px-4 text-center border-b border-emerald-100/50 bg-white/50">
          <p className="font-serif text-3xl text-emerald-900 leading-loose">بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ</p>
        </div>
      )}
      ...
      <div className="px-4 mt-4 space-y-4">
        {surah?.ayahs.map((ayah) => (
          <div key={ayah.number} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            {/* Ayah Header */}
            <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-50">
              <div className="flex items-center space-x-3">
                <div className="relative flex items-center justify-center w-8 h-8">
                  <div className="absolute inset-0 bg-emerald-50 rounded-full"></div>
                  <span className="relative z-10 text-emerald-700 font-bold text-xs">
                ...
                <div className="flex space-x-2">
                  <button 
                    ...
                    className={cn(
                      ...
                      playingAyah === ayah.number ? "bg-emerald-100 text-emerald-700" : "bg-slate-50 text-slate-500 hover:bg-slate-100"
                    )}
                  >
